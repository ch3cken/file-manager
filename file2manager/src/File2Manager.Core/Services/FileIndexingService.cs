...
public sealed class FileIndexingService : IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _databaseService.InitializeAsync(config, cancellationToken, backfillQuickTerms: false);
    ...
    private FileIndexRecord? CreateIndexRecord(string filePath, IndexingScope scope)
    {
        ...
        var record = new FileIndexRecord
        {
        ...
        if (isSmartScope)
        {
            var embedding = _embeddingService.EmbedRecord(record);
            if (embedding is { Length: > 0 })
            {
                record.EmbeddingModel = SemanticEmbeddingService.CurrentModelName;
                record.EmbeddingVector = _embeddingService.Serialize(embedding);
            }
        }
