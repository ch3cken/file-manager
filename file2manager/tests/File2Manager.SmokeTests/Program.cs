...
try
{
    ...
    var automobilePolicy = Path.Combine(documentsRoot, "automobile-policy.txt");
    var pathOnlyMachineRoot = Path.Combine(documentsRoot, "machine-folder");
    var pathOnlyMachineFile = Path.Combine(pathOnlyMachineRoot, "unrelated-note.txt");

    Directory.CreateDirectory(pathOnlyMachineRoot);
    ...
    await File.WriteAllTextAsync(automobilePolicy, "Automobile insurance coverage, vehicle premiums, collision liability, and policy deductibles.");
    await File.WriteAllTextAsync(pathOnlyMachineFile, "This file only matches machine through its parent folder name.");
    ...
    Expect(allRecords.Any(record =>
        record.IsSmartIndexed &&
        record.EmbeddingModel == SemanticEmbeddingService.CurrentModelName &&
        record.EmbeddingVector.Length > 0),
        "smart-indexed files should store semantic embedding vectors");

    var searchService = new SearchService(databaseService, new SearchQueryParser());
    var dateWordsOnlyQuery = searchService.Parse("machine learning paper last week");
    Expect(!dateWordsOnlyQuery.HasDateFilter, "date filters should come from the UI menu, not query inference");
    ...
    Expect(quickResults.First().FileName == "machine-learning-paper.txt", "file name prefix matches should outrank path-only prefix matches");
    ...
    var cappedBulkResults = await searchService.QuickSearchAsync("bulk", 100);
    Expect(cappedBulkResults.Count <= 20, "quick search should cap results at 20 files");
    ...
    var synonymResults = await searchService.SmartSearchAsync("car", 10);
    Expect(synonymResults.Any(result => result.FileName == "automobile-policy.txt"), "embedding smart search did not find a synonym-related automobile file for car");
